async function handleWebSocketGameMessage(
  ...
  */
  ...
  if(m.type==="state"){
    ...
    if(gameType==="course"){
      const localRuntime=
        Array.isArray(previousState?.runtimeBlocks)
          ?previousState.runtimeBlocks
          :createRuntimeBlocks();

      remoteState={
        ...m.state,
        runtimeBlocks:localRuntime
      };

      setCourseClockAnchor(remoteState.elapsed||0);
